...
module.exports = {
   ...
   async findAll(req, res) {
      ...
      let result = await Promise.all([info(), link()]);
      const infos = result[0];
      const links = result[1];

      // Instância array
      const universities = [];

      // Percorre os dois arrays juntando ao encontrar campuses com nomes iguais
      infos.map((info) => {
         links.map((att) => {
            // Se forem iguais é atribuido o valor do link
            if (att.campuses === info.campuses) {
               info = {...info, link: att.link};
