...
const LinkOntario = (async function LinkOntario() {
   // Abre a página
   ...
   await page.$eval('input[type="search"]', el => el.value = 'Ontario');
   ...
   fs.writeFile('./src/json/LinkOntario.json', JSON.stringify(infoArray, null, 2), err => {
      if (err) throw new Error('There was an error in links Ontario', err);
      console.log('Recorded links Ontario successfully');
   });
   // Fecha browser
   await browser.close();
});

module.exports = LinkOntario;
